...
Promise.resolve()
    ...
    .then(process => process ? worqr.stopProcess(process.processName) : Promise.resolve())
    .then(() => worqr.startTask('myQueue1'))
    // stop work on queue 2
    .then(() => worqr.stopWork('myQueue2'))
    // test finishing a task
    .then(() => worqr.startTask('myQueue3'))
    .then(process => process ? worqr.finishProcess(process.processName) : Promise.resolve())
